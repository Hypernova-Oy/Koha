...
<xsl:stylesheet version="1.0"
 ...
 <xsl:variable name="SubjectModifier"><xsl:if test="marc:sysprefs/marc:syspref[@name='TraceCompleteSubfields']='1'">,fullständigt-delfält</xsl:if></xsl:variable>
 ...
 <xsl:choose>
 <xsl:when test="$leader19='a'">Ställ in</xsl:when>
 <xsl:when test="$leader6='a'">
 <xsl:choose>
 <xsl:when test="$leader7='c' or $leader7='d' or $leader7='m'">Text</xsl:when>
 <xsl:when test="$leader7='i' or $leader7='s'">
 <xsl:choose>
 <xsl:when test="substring($controlField008,22,1)!='m'">Periodika</xsl:when>
 <xsl:otherwise>Serie</xsl:otherwise>
 </xsl:choose>
 </xsl:when>
 <xsl:when test="$leader7='a' or $leader7='b'">Artikel</xsl:when>
 </xsl:choose>
 </xsl:when>
 ...
 <xsl:when test="$leader6='m'">Datafil</xsl:when>
 <xsl:when test="$leader6='e' or $leader6='f'">Karta </xsl:when>
 ...
 <xsl:when test="$leader6='i'">Ljud</xsl:when>
 <xsl:when test="$leader6='c' or $leader6='d'">Musiktryck</xsl:when>
 </xsl:choose>
 </xsl:variable>
 ...
 <xsl:choose>
 <xsl:when test="$materialTypeLabel='Book'">Bok</xsl:when>
 <xsl:when test="$materialTypeLabel='Map'">Karta </xsl:when>
 <xsl:when test="$materialTypeLabel='Music'">Album</xsl:when>
 ...
 <xsl:attribute name="typeof"><xsl:value-of select='$schemaOrgType' /> Produkt</xsl:attribute>
 <xsl:attribute name="resource">#post</xsl:attribute>
 ...
 <span class="label">Editionsnummer: </span>
 ...
 <span class="results_summary volumes"><span class="label">Volymer: </span>
 <a>
 <xsl:choose>
 <xsl:when test="$UseControlNumber = '1' and marc:controlfield[@tag=001]">
 ...
 <xsl:text>Visa volymer</xsl:text>
 </a>
 </span>
 </xsl:if>
 ...
 <span class="results_summary set"><span class="label">Ställ in: </span>
 ...
 <xsl:with-param name="label">Utgivningsinformation: </xsl:with-param>
 </xsl:call-template>
 </xsl:if>
 ...
 <span class="results_summary publisher"><span class="label">Utgivningsinformation: </span>
 ...
 <xsl:with-param name="label">Utgåva: </xsl:with-param>
 </xsl:call-template>
 </xsl:if>

 <xsl:if test="marc:datafield[@tag=250]">
 <span class="results_summary edition"><span class="label">Utgåva: </span>
 ...
 <span class="results_summary description"><span class="label">Beskrivning: </span>
 ...
 <span class="label">Innehållstyp:  </span>
 ...
 <span class="label">Medietyp: </span>
 <ul class="resource_list">
 ...
 <span class="label">Bärartyp: </span>
 <ul class="resource_list">
 ...
 <span class="label">Målgrupp: </span>
 <ul class="resource_list">
 ...
 <span class="label">Rapportnummer:</span>
 ...
 <xsl:with-param name="label">Annan titel: </xsl:with-param>
 </xsl:call-template>
 </xsl:if>

 <xsl:if test="marc:datafield[@tag=246]">
 <span class="results_summary other_title">
 <span class="label">Annan titel: </span>
 <ul class="resource_list">
 ...
 <xsl:choose>
 <xsl:when test="@ind2=0"> [Del av titel]</xsl:when>
 <xsl:when test="@ind2=1"> [Parallelltitel]</xsl:when>
 <xsl:when test="@ind2=2"> [Distinkt titel]</xsl:when>
 <xsl:when test="@ind2=3"> [Annan titel]</xsl:when>
 <xsl:when test="@ind2=4"> [Omslagstitel] </xsl:when>
 <xsl:when test="@ind2=5"> [Tillagd rubrik på titelsida]</xsl:when>
 <xsl:when test="@ind2=6"> [Rubriktitel]</xsl:when>
 <xsl:when test="@ind2=7"> [Löpande titel]</xsl:when>
 <xsl:when test="@ind2=8"> [Ryggtitel] </xsl:when>
 </xsl:choose>
 </xsl:if>
 </span>
 </li>
 ...
 <span class="results_summary translated_title"><span class="label">Översatt titel: </span>
 ...
 <xsl:with-param name="label">Originaltitel: </xsl:with-param>
 </xsl:call-template>
 </span>
 </xsl:if>
 ...
 <span class="label">Relaterade verk: </span>
 <ul class="resource_list">
 ...
 <span class="label">Inkluderade verk: </span>
 <ul class="resource_list">
 ...
 <xsl:with-param name="delimeter"> OCH </xsl:with-param>
...
<!-- MARC21 776 Additional Physical Form Entry -->
 ...
 <span class="label">Fler format: </span>
...
<!-- DDC classification -->
 ...
 <span class="label">DDK-klassifikation: </span>
 <ul class="resource_list">
...
<!-- LOC classification -->
 ...
 <span class="label">Library of Congress (LC) klassifikationskod: </span>
 <ul class="resource_list">
...
<!-- Other classification -->
 ...
 <span class="label">Annan klassifikation: </span>
 <ul class="resource_list">
 ...
 <span class="label">Onlineresurser: </span>
 <ul class="resource_list">
 ...
 <xsl:otherwise>
 <xsl:text>Klicka här för att gå online</xsl:text>
 </xsl:otherwise>
 </xsl:choose>
 </a>
 </li>
 ...
 <span class="results_summary other_relationship_entry"><span class="label">Andra relaterade verk: </span>
 ...
 <span class="label">Ofullständigt innehåll: </span>
 </xsl:when>
 <xsl:when test="@ind1=2">
 <span class="label">Delvis innehåll:</span>
 </xsl:when>
 <xsl:otherwise>
 ...
 <span class="label">Åtgärdsanteckning: </span>
 <ul class="resource_list">
 ...
 <span class="label">Medverkande i produktionen: </span>
 <ul class="resource_list">
 ...
 <span class="label">Utmärkelser:  </span>
 </xsl:if>
 <ul class="resource_list">
 ...
 <span class="label">
 <xsl:if test="@ind1=1"><xsl:text>Rollbesättning:  </xsl:text></xsl:if>
 </span>
 <xsl:call-template name="subfieldSelect">
 <xsl:with-param name="codes">a</xsl:with-param>
 </xsl:call-template>
 ...
 <span class="label">Avhandlingskommentar: </span>
 ...
 <span class="label">
 <xsl:choose>
 <xsl:when test="@ind1=0"><xsl:text>Ämne: </xsl:text></xsl:when>
 <xsl:when test="@ind1=1"><xsl:text>Recension </xsl:text></xsl:when>
 <xsl:when test="@ind1=2"><xsl:text>Omfattning och innehåll: </xsl:text></xsl:when>
 <xsl:when test="@ind1=3"><xsl:text>Abstrakt: </xsl:text></xsl:when>
 <xsl:when test="@ind1=4"><xsl:text>Vägledning för innehåll:  </xsl:text></xsl:when>
 <xsl:otherwise><xsl:text>Sammanfattning: </xsl:text></xsl:otherwise>
 </xsl:choose>
 </span>
 <xsl:call-template name="subfieldSelect">
 <xsl:with-param name="codes">abc</xsl:with-param>
 ...
 <span class="results_summary holdings_note basic_bibliographic_unit"><span class="label">Bestånd: </span>
 ...
 <span class="results_summary holdings_note supplementary_material"><span class="label">Bilagor: </span>
 ...
 <span class="results_summary holdings_note indexes"><span class="label">Index: </span>
 ...
 <span class="label">Översättning av: </span>
 </xsl:when>
 <xsl:otherwise>
 <span class="label">
 <xsl:value-of select="marc:subfield[@code='i']"/>
 ...
 <span class="label">Bilaga till: </span>
 <ul class="resource_list">
 ...
 <span class="results_summary other_editions"><span class="label">Andra utgåvor: </span>
 ...
 <xsl:choose>
 <xsl:when test="@ind2=0">
 <span class="label">Fortsätter:</span>
 </xsl:when>
 <xsl:when test="@ind2=1">
 <span class="label">Fortsätter delvis:</span>
 </xsl:when>
 <xsl:when test="@ind2=2">
 <span class="label">Ersätter:</span>
 </xsl:when>
 <xsl:when test="@ind2=3">
 <span class="label">Ersätter delvis:</span>
 </xsl:when>
 <xsl:when test="@ind2=4">
 <span class="label">Skapad av föreningen: ... och: ...</span>
 </xsl:when>
 <xsl:when test="@ind2=5">
 <span class="label">Absorberad:</span>
 </xsl:when>
 <xsl:when test="@ind2=6">
 <span class="label">Delvis absorberad:</span>
 </xsl:when>
 <xsl:when test="@ind2=7">
 <span class="label">Separat från:</span>
 </xsl:when>
 </xsl:choose>
 <xsl:text> </xsl:text>
 ...
 <xsl:choose>
 <xsl:when test="@ind2=0">
 <span class="label">Fortsättes av:</span>
 </xsl:when>
 <xsl:when test="@ind2=1">
 <span class="label">Fortsättes delvis av:</span>
 </xsl:when>
 <xsl:when test="@ind2=2">
 <span class="label">Ersatt av:</span>
 </xsl:when>
 <xsl:when test="@ind2=3">
 <span class="label">Delvis ersatt av:</span>
 </xsl:when>
 <xsl:when test="@ind2=4">
 <span class="label">Absorberad av:</span>
 </xsl:when>
 <xsl:when test="@ind2=5">
 <span class="label">Delvis absorberad av:</span>
 </xsl:when>
 <xsl:when test="@ind2=6">
 <span class="label">Dela upp i .. och ...:</span>
 </xsl:when>
 <xsl:when test="@ind2=7">
 <span class="label">Sammanslagen med ... för att bilda ...</span>
 </xsl:when>
 <xsl:when test="@ind2=8">
 <span class="label">Ändrad tillbaka till:</span>
 </xsl:when>
 </xsl:choose>
 <xsl:text> </xsl:text>
 ...
 <xsl:text> Spela upp detta ljudprov</xsl:text>
 </button>
 </div>
 </xsl:if>
 </xsl:if>
 ...
 <!-- #13383 -->
 <xsl:when test="@tag&lt;700"><span class="byAuthor">Av: </span></xsl:when>
 <!--#13382 Changed Additional author to contributor -->
 <xsl:otherwise>Medverkande: </xsl:otherwise>
 </xsl:choose>
 </xsl:for-each>
 <ul class="resource_list">
 <xsl:for-each select="$authorfield">
 ...
 <xsl:attribute name="class">medverkande</xsl:attribute>
 <span resource="#record"><span>
 <xsl:choose>
 <xsl:when test="substring(@tag, 1, 1)='1'">
 <xsl:choose>
 <xsl:when test="$materialTypeLabel='Music'"><xsl:attribute name="property">avUpphovsman</xsl:attribute></xsl:when>
 <xsl:otherwise><xsl:attribute name="property">författare </xsl:attribute></xsl:otherwise>
 </xsl:choose>
 </xsl:when>
 <xsl:otherwise><xsl:attribute name="property">medverkande</xsl:attribute></xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
 <xsl:when test="substring(@tag, 2, 1)='0'">
 <xsl:choose>
 <xsl:when test="$materialTypeLabel='Music'"><xsl:attribute name="typeof">Musikgrupp</xsl:attribute></xsl:when>
